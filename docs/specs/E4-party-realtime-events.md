...
- 채팅 화면도 같은 파티의 공개 상태 변경을 감지해 방 목록과 상세 정보를 다시 동기화한다.
...
### `party-updated`
- 의미: 파티 생성, 조건 변경, 참여 확정, 취소, 대기열 승격, 마감으로 공개 상태가 바뀌었다.
- payload
...
- 채팅
  - 현재 참여 중인 채팅방의 파티 ID와 이벤트 `id`가 일치하면 `/api/chat/rooms`를 다시 조회한다.
  - 현재 보고 있는 채팅방과 이벤트 `id`가 일치하면 `/api/chat/rooms/{partyId}`를 다시 조회해 오픈채팅 링크와 제목 변경을 반영한다.
