...
- 대기열 자동 승격도 같은 차단 규칙을 다시 검사하고, 차단 관계가 생긴 대기자는 `EXPIRED`로 종료한 뒤 다음 대기자를 계속 본다.
...
- 긍정적
  ...
  - 대기열에 먼저 들어온 사용자도 승격 시점에 현재 차단 상태를 다시 반영하므로 자동 참여 우회가 줄어든다.
...
- 부정적
  ...
  - 승격 전에 차단된 대기자는 별도 알림 없이 `EXPIRED` 상태가 되므로, 현재는 사용자가 목록 새로고침이나 재조회 시점에만 상태 변화를 확인한다.
- 중립적
